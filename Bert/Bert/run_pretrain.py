...
def run_pretrain():
    ...
    if cfg.accumulation_steps > 1:
        logger.info("累积步数: {}".format(cfg.accumulation_steps))
        logger.info("全局批量大小: {}".format(cfg.batch_size * cfg.accumulation_steps))
        if cfg.enable_data_sink == "true":
            cfg.data_sink_steps *= cfg.accumulation_steps
            logger.info("数据下沉步数: {}".format(cfg.data_sink_steps))
        if cfg.enable_save_ckpt == "true":
            cfg.save_checkpoint_steps *= cfg.accumulation_steps
            logger.info("保存检查点步数: {}".format(cfg.save_checkpoint_steps))
    ...
    # 如果需要训练时评估，则创建评估网络和数据集，并添加评估回调函数
